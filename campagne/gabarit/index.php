...
<!-- <a href="edit/majPos.php" class="btn btn-success btn-lg majPos"><span class="glyphicon glyphicon-plus"></span>Modifier</a> -->
...
$statement = $db->query('SELECT * FROM tranche JOIN background  WHERE tranche.campagne ="'.$nameFolder.'" ORDER BY top DESC LIMIT 1 ');
...
while($item = $statement->fetch())
{
    ...
    $filename= $base.'/'.$nameFolder.'/img/'.$item['image'];
...
while($item = $statement->fetch())
{
    ...
    echo'<div id="" class="test " style="top:'.$item['top'].'px; position: absolute; margin-left: -15px"><img class="star" id="'.$item['id'].'" src="img/'.$item['image'].'" alt=""></div>';
